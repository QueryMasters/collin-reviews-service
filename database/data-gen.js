  ...
  function write() {
    ...
    do {
      ...
      if (i === 0) {
        ...
        // don't pass the callback, because we're not done yet.
        ok = writer.write(
          `${faker.internet.userName()}\t${faker.image.avatar()}\n`
        )
      }
    } while (i > 0 && ok)
    if (i > 0) {
  ...
  function write() {
    ...
    do {
      ...
      if (i === 0) {
        // last time!
        writer.write(
          `${faker.company.catchPhrase()}\t${faker.lorem.paragraph()}\t${faker.random.number(
            { max: 6, min: 1 }
          )}\t${
            faker.date
              .past()
            ...
            max: 1000,
            min: 0
          })}\t${faker.random.boolean()}\t${faker.random.number({
            max: 10000000,
            min: 1
          })}\t${faker.random.number({ max: 10000000, min: 1 })}\n`
        )
      } else {
        // see if we should continue, or wait
        // don't pass the callback, because we're not done yet.
  ...
  function write() {
    ...
    do {
      ...
      if (i === 0) {
        // last time!
        writer.write(
          `${faker.commerce.productAdjective()}\t${faker.random.number({
            max: 6,
            min: 1
          })}\t${faker.random.number({
            max: 1000,
  ...
  function write() {
    ...
    do {
      ...
      if (i === 0) {
        // last time!
        writer.write(
          `photo\t${faker.image.image()}\t${faker.random.number({
            max: 10000000,
            min: 1
          })}\n`
        )
